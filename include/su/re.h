 ...
 * matches).
 * Upon failure \r{su_re::re_error} can be inspected, and \r{su_re_error_doc()}
 * might be used. */
EXPORT boole su_re_setup_cp(struct su_re *self, char const *expr,
...
private:
   CTA(sizeof(match) == sizeof(struct su_re_match),
      "Wild match==su_re_match cast constraint not fulfilled");
public:
   ...
   boole setup(char const *expr, BITENUM_IS(u8,setup_flags) flags=setup_ext){
      return su_re_setup_cp(this, expr, S(u8,flags));
   }
